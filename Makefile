...
all: osmetrics-cpu.so osmetrics-network.so osmetrics-loadavg.so osmetrics-swapinfo.so osmetrics-memory.so osmetrics-vmstat.so osmetrics-cpuinfo.so osmetrics-mounts.so osmetrics-misc.so \
     osmetrics-diskstats.so osmetrics-version.so osmetrics-ioscheduler.so osmetrics-meminfo.so osmetrics-cpugovernor.so osmetrics-stat.so
...
osmetrics-stat.so: osmetrics-stat.c
        $(CC) $(CFLAGS) -o osmetrics-stat.so osmetrics-stat.c
...
clean:
	...
	rm -f ${PLUGINDIR}/osmetrics-stat.so
...
install:
	...
	cp -f osmetrics-stat.so ${PLUGINDIR}
